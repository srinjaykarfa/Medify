...
def book_appointment(appointment: AppointmentRequest, current_user: dict = Depends(get_current_user)):
...
def get_my_appointments(current_user: dict = Depends(get_current_user)):
    ...
    try:
        appointments = list(db.appointments.find(
            {"patient_id": current_user["id"]},
            {"_id": 1, "doctor_name": 1, "doctor_specialization": 1, 
             "appointment_date": 1, "appointment_time": 1, "status": 1, "notes": 1, "created_at": 1}
        ))
