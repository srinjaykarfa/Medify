  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'reports':
        return (
          <div className="text-center py-20">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Reports Section</h2>
      ...
      case 'analytics':
        return (
          <div className="text-center py-20">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Analytics Section</h2>
      ...
      case 'settings':
        return (
          <div className="text-center py-20">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Settings Section</h2>
...
export default DoctorDashboard;
  ...
  useEffect(() => {
    ...
    if (!token || userRole !== 'doctor') {
      toast.error('Authentication required. Please login as a doctor.');
      navigate('/signin');
      return;
    }
  ...
  const fetchDoctorData = async () => {
    ...
    try {
      ...
      if (statsResponse.ok) {
        ...
        console.log('Stats API failed, using empty data');
        setStats({
          todayAppointments: 0,
          totalPatients: 0,
      ...
      if (appointmentsResponse.ok) {
        ...
        console.log('Appointments API failed, using empty data');
        setAppointments([]);
      }
      ...
      if (patientsResponse.ok) {
        ...
        console.log('Patients API failed, using empty data');
        setPatients([]);
      }
  ...
  const handleAppointmentStatus = async (appointmentId, newStatus) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/doctor/appointments/${appointmentId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const acceptAppointment = (appointmentId) => {
    handleAppointmentStatus(appointmentId, 'confirmed');
  };
  ...
  const completeAppointment = (appointmentId) => {
    handleAppointmentStatus(appointmentId, 'completed');
  };
          ...
          >
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/30">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                <motion.div 
                  className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-2xl border border-blue-200"
                  whileHover={{ scale: 1.02 }}
                >
                  <h4 className="font-bold text-blue-800 text-lg">
                    {appointments.filter(apt => apt.status === 'pending').length}
                  </h4>
                  <p className="text-blue-600 text-sm font-medium">Pending Approval</p>
                </motion.div>
                <motion.div 
                  className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-2xl border border-green-200"
                  whileHover={{ scale: 1.02 }}
                >
                ...
                <motion.div 
                  className="bg-gradient-to-r from-purple-50 to-purple-100 p-4 rounded-2xl border border-purple-200"
                  whileHover={{ scale: 1.02 }}
                >
                ...
                <motion.div 
                  className="bg-gradient-to-r from-red-50 to-red-100 p-4 rounded-2xl border border-red-200"
                  whileHover={{ scale: 1.02 }}
                >
              ...
              <div className="space-y-4">
                {appointments.length > 0 ? appointments.map((appointment, index) => (
                  <motion.div
                    key={appointment.id}
                    className="bg-gradient-to-r from-gray-50 to-white p-6 rounded-2xl border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300"
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.1 * index }}
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        {appointment.status === 'pending' && (
                          <div className="flex space-x-3">
                            <motion.button
                              ...
                              className="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-xl hover:from-green-600 hover:to-green-700 transition-all duration-300 font-semibold shadow-lg flex items-center space-x-2"
                            ...
                            <motion.button
                              ...
                              className="bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-3 rounded-xl hover:from-red-600 hover:to-red-700 transition-all duration-300 font-semibold shadow-lg flex items-center space-x-2"
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                            >
                        ...
                        {appointment.status === 'confirmed' && (
                          <motion.button
                            ...
                            className="bg-gradient-to-r from-purple-500 to-purple-600 text-white px-6 py-3 rounded-xl hover:from-purple-600 hover:to-purple-700 transition-all duration-300 font-semibold shadow-lg flex items-center space-x-2"
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                          >
                            <CheckCircleIcon className="w-5 h-5" />
                        ...
                        {(appointment.status === 'completed' || appointment.status === 'rejected') && (
                          <div className="flex items-center space-x-2 text-gray-500">
                            <CheckCircleIcon className="w-5 h-5" />
                            <span className="text-sm font-medium">No actions available</span>
                          </div>
                        )}
                      </div>
                    </div>
